...
trial_mm_role_id = 1199245904100724746
trial_exch_role_id = 1199245905132535829
...
async def client(ctx, user: discord.User):
    client_role_id = 1188180972307484752
...
async def ehelp(ctx):
    ...
    embed.add_field(name=".restrict", value="It Will Restrict To The User")
    embed.add_field(name=".unrestrict", value="It Will Unrestrict to the user")
    embed.add_field(name=".addstaff", value="SOON")
    embed.add_field(name=".removestaff", value="SOON")
...
async def rminr(ctx, amount:float):
    user = ctx.author
    embed = discord.Embed(
        description=f'**<a:green_tick:1183021746115326023> {user.mention} HAS RECEIVED â‚¹{amount}\n <a:green_tick:1183021746115326023> NOW YOU CAN CONTINUE YOUR DEAL.\n <a:green_tick:1183021746115326023> PING {user.mention} TO RELEASE**',
        color=0x000000)
    await ctx.send(embed=embed)
    embed.set_footer(text="Legitimacy Service", icon_url="https://media.discordapp.net/attachments/1199245971381559326/1199574180123922493/20240123_115328_0000.png?ex=65c30983&is=65b09483&hm=cc34d0180cfac5d9db4acd587ba447891da7d249e229bfed51700db68fcab582&")
    await ctx.message.delete()
...
async def rmcrypto(ctx,amount:float):
    user = ctx.author
    embed = discord.Embed(
        description=f'**<a:green_tick:1183021746115326023> {user.mention} HAS RECEIVED ${amount}\n <a:green_tick:1183021746115326023> YOU CAN CONTINUE YOUR DEAL.\n <a:green_tick:1183021746115326023> PING {user.mention} TO RELEASE**',
        color=0x000000)
    embed.set_footer(text="Legitimacy Service", icon_url="https://media.discordapp.net/attachments/1199245971381559326/1199574180123922493/20240123_115328_0000.png?ex=65c30983&is=65b09483&hm=cc34d0180cfac5d9db4acd587ba447891da7d249e229bfed51700db68fcab582&")
    await ctx.send(embed=embed)
    await ctx.message.delete()
...
@commands.has_role(STAFF)
async def pyn(ctx):
  paynote = "** Paynote is Compulsory to add while making payment if you will not add we will take penalty . \n Penalty 10 Rs \n **Paynote** \n â€¢ For  Fampay users : I have received my products .\n â€¢ For other UPI users : I authorized this payment and received my products. **"
